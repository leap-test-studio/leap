...
router.get("/:tenantId/role/:role", csrf, Authorizer(RoleGroups.Admins), getAllAccountsByRole);
router.get("/", csrf, Authorizer(RoleGroups.Admins), getAllAccounts);

router.get("/:id", Authorizer(), getById);
router.post("/", Authorizer(RoleGroups.Admins), accountSchema, create);
router.put("/:id", Authorizer(RoleGroups.Admins), updateAccountSchema, update);
...
function revokeToken(req, res) {
    ...
    if (!req.auth.ownsToken(token) && req.auth.role !== E_ROLE.Admin) {
        return res.status(status.UNAUTHORIZED).send({
...
function getById(req, res) {
    ...
    if (Number(req.params.id) !== req.auth.id && req.auth.role !== E_ROLE.Admin) {
...
function update(req, res) {
    ...
    if (Number(req.params.id) !== req.auth.id && req.auth.role !== E_ROLE.Admin && req.auth.role !== E_ROLE.Lead) {
        return res.status(status.UNAUTHORIZED).send({ message: status[`${status.UNAUTHORIZED}_MESSAGE`] });
    }
