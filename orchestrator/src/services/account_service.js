...
const { E_ROLE } = require("engine_utils");
...
async function getAllAccountsByRole({ page, size, token, role, tenantById }) {
    ...
    let accounts, data;
    if ([E_ROLE.Manager].includes(role)) {
...
function register(params) {
    return new Promise(async (resolve) => {
        ...
        if (isFirstAccount) {
            account.role = E_ROLE.Admin;
            account.verificationToken = randomTokenString();
        } else {
            account.role = E_ROLE.Engineer;
...
async function registerOktaUser(req) {
    ...
    if (!account && auth.iss === global.config.OKTA_ISSUER) {
...
async function create(params, userID = null) {
    ...
    if (params.role === E_ROLE.Admin) {
        Object.assign(params, { addedbyId: userID });
    }

    if (params.role === E_ROLE.Engineer) {
...
async function update(id, params, userID, userRole) {
    ...
    if (userRole === E_ROLE.Engineer && account.managerId !== userID) {
...
async function sendOnboardEmail(account, password) {
    try {
        return await sendEmail(
            account.email,
            [E_ROLE.Manager, E_ROLE.Lead, E_ROLE.Engineer].includes(account.role) ? "welcome-manager.html" : "welcome-admin.html",
            {
...
async function sendVerificationEmail(account) {
    ...
    return await sendEmail(
        account.email,
        account.role === E_ROLE.Admin ? "welcome.html" : "verification-code.html",
        {
