...
const { E_EXEC_STATE, E_SLEEP_TIMING_TYPE } = require("engine_utils");
...
class TestRunner extends Runner {
        ...
        if (event.enabled && this.shouldTaskContinue()) {
            ...
            try {
                if (ActionsTypes.includes(event.actionType)) {
                    if (this.sleepTimingType === E_SLEEP_TIMING_TYPE.Before || this.sleepTimingType === E_SLEEP_TIMING_TYPE.BeforeAndAfter) {
                        logger.info(
                    ...
                    logger.info(
                        "WebTestRunner: Step[" + stepOutcome.stepNo + "] Executing:",
                        event.actionType,
                        ", Payload:",
                    ...
                    logger.info(
                        "WebTestRunner: Step[" + stepOutcome.stepNo + "] Outcome:",
                        event.actionType,
                        ", Outcome:",
                        JSON.stringify(hanndlerOutput)
                    );
                    stepOutcome = merge({}, stepOutcome, hanndlerOutput);
                    logger.info(
                        "WebTestRunner: Step[" + stepOutcome.stepNo + "] Merged Outcome",
                        event.actionType,
                        ", Outcome:",
                        JSON.stringify(stepOutcome)
                    );
                    if (this.sleepTimingType === E_SLEEP_TIMING_TYPE.After || this.sleepTimingType === E_SLEEP_TIMING_TYPE.BeforeAndAfter) {
                        logger.info(
            ...
            if (stepOutcome.result === E_EXEC_STATE.FAIL) {
    ...
    async execute() {
        ...
        const outcome = this.steps.filter((s) => s != null).find((s) => s.result === E_EXEC_STATE.FAIL) || { result: E_EXEC_STATE.PASS };

        logger.info("WebTestRunner: Executed All Steps:", JSON.stringify(outcome));
        this.actual = { actualResult: outcome };
        this.result = outcome?.result;
        if (this.result === E_EXEC_STATE.PASS) {
    ...
    async getActionHandler(actionType, data) {
        ...
        const Handlers = {
            ...
            verifyElementHasTextValue: async ({ by, element, value }) => {
                ...
                return {
                    actual: text,
                    result: text?.includes(value) ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
            verifyElementVisible: async ({ by, element }) => {
                ...
                return {
                    actual: isDisplayed,
                    result: isDisplayed ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
            ...
            verifyElementAttributeValue: async ({ by, element, value, attribute = "value" }) => {
                ...
                return {
                    actual: attrValue,
                    result: value === attrValue ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
            verifyElementChecked: async ({ by, element }) => {
                ...
                return {
                    actual: isSelected,
                    result: isSelected ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
            ...
            verifyElementClickable: async ({ by, element }) => {
                ...
                return {
                    ...
                    result: typeof webElement.click === "funciton" ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
            ...
            verifyElementHasAttribute: async ({ by, element, attribute }) => {
                ...
                return {
                    actual: attr,
                    result: attr !== null ? E_EXEC_STATE.PASS : E_EXEC_STATE.FAIL
                };
            },
