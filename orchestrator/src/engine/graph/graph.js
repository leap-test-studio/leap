const { NodeAlreadyExistsError, NodeDoesntExistError } = require("./errors");
...
module.exports = class Graph {
  constructor(nodeIdentity = (node) => hash(node)) {
    this.nodes = new Map();
    this.adjacency = [];
   ...
   */
  addEdge(node1Identity, node2Identity) {
   ...
   *
   * @param id An optional node id that indicates the sort order of the returned array
   */
  getNodes(id) {
    const temp = Array.from(this.nodes.values());

    if (id) {
      return temp.sort((n) => n.id === id);
    }
