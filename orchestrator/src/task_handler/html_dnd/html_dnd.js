...
(function (dnd) {
  ...
  function simulate(draggable, droppable, config = {}) {
    if (config?.onlyHover) {
      simulateDragOver(draggable, droppable, config);
    } else {
      simulateDragAndDrop(draggable, droppable, config);
    }
  }
  dnd.simulate = simulate;
  function simulateDragAndDrop(draggable, droppable, config) {
    ...
    drop(droppable, dataTransfer, config);
    endDrag(store, draggable, dataTransfer);
  }
  dnd.simulateDragAndDrop = simulateDragAndDrop;
  function simulateDragOver(draggable, droppable, config) {
    if (!config.hoverTime) {
      config.hoverTime = 0;
    }
    const store = new DragDataStore();
  ...
  function drop(droppable, dataTransfer, config) {
    const dropPosition = calculateMousePositions(draggable, config?.dropOffset);
    const dropEvent = createEventWithDataTransfer("drop", dataTransfer, dropPosition);
   ...
   */
  function createEventWithDataTransfer(type, dataTransfer, position) {
    let args = [type, true, true, window];
    if (position) {
      args = args.concat([0, 0, 0, position.x, position.y, false, false, false, false, 0, null]);
    }
    const mouseEvent = document.createEvent("MouseEvent");
    mouseEvent.initMouseEvent.apply(mouseEvent, args);
