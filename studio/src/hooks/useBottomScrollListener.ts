 ...
 */
export const useBottomScrollListener = function <T extends HTMLDivElement>(
  onBottom: () => void,
  options?: {
    offset?: number;
    debounce?: number;
  ...
  const debouncedOnBottom = useMemo(() => createCallback(debounce, onBottom, debounceOptions), [debounce, onBottom, debounceOptions]);
