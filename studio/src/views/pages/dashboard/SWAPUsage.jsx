...
const SWAPUsage = ({ swapInfo }) => {
  ...
  let availableRam = 0;
  if (swapInfo) {
    total = Number(swapInfo.swapTotal);
    available = Number(swapInfo.swapFree);
    availableRam = Math.ceil(available / (1024 * 1024 * 1024));
    totalRam = Math.ceil(total / (1024 * 1024 * 1024));
  }
  if (total > 0) {
    usageRam = Math.ceil((total - available) / (1024 * 1024 * 1024));
  }
  ...
  return (
    <>
      <div className="font-semibold text-color-label text-base text-center pt-4 select-none">SWAP Usage</div>
      {!isNaN(total) ? (
