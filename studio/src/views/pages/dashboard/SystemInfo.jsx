const SystemInfo = ({ info, noOfnetworkInterfaces }) => {
  ...
  return (
    <div className="px-2 py-4">
      <div className="font-semibold text-slate-700 text-lg mb-4 select-none">System Information</div>
      <div className="font-semibold text-sm mb-1">CPU</div>
      ...
      <div className="font-semibold text-sm mb-1">HOSTNAME</div>
      {info.hostname !== undefined ? (
        <p className="text-slate-500 font-semibold text-xs mb-4">{info.hostname}</p>
      ) : (
        <div className="bg-slate-200 w-full animate-pulse h-3 rounded-2xl mb-4" />
      )}
      <div className="font-semibold text-sm mb-1">OS</div>
      {info.os !== undefined ? (
