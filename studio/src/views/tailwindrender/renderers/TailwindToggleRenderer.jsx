...
const TailwindToggleRenderer = ({ data, uischema, path, handleChange, config, visible, enabled }) => {
  ...
  return (
    <Switch
      ...
      className={`mx-2 group relative flex h-5 w-10 cursor-pointer rounded-full bg-gray-300 p-1 transition-colors duration-200 ease-in-out focus:outline-none data-[focus]:outline-1 data-[focus]:outline-white ${enabled ? "data-[checked]:bg-color-0300" : "cursor-not-allowed"}`}
    >
      <span
        aria-hidden="true"
